...
def main():
    ...
    while maxMove < 20 or maxMove > 200:
    ...
    while collisions <= maxCollisions:
        print(f'Outer loop: distance={distance}, maxMove={maxMove}, collision={collision}')
        ...
        while (distance < maxMove) and (not collision):
            ...
            print(f"Check 4, collision={collision}")
            if not collision:
                collision = detectCollisionWithCircle(bob, circle3)
            print(f"Check 5, collision={collision}")
            if not collision:
                collision = detectCollisionWithCircle(bob, circle4)
            print(f"Check 6, collision={collision}")
            # TODO 7: add collision detection for rectangle2
            if not collision:
                collision = detectCollisionWithRectangle(bob, rectangle2)
            print(f"Check 7, collision={collision}")

            print("Continue loop? collision={}", collision)
        ...
        if distance == maxMove:
            maxMove = int(input("Enter maximum distance to travel between moves (20-200 pixels): "))
            direction = getNewHeading(bob)
            bob.setheading(direction)
            distance = 0
            print(f"Check distance, distance={distance}, maxMove={maxMove}, collisions={collision}")
        ...
        # otherwise, turtle must have collided with an obstacle
            ...
            collisions += 1
            print(f"Check distance, distance={distance}, maxMove={maxMove}")
            print(f"A collision has occurred. Oops! Your total collisions = {collisions}!")

            # if reached end of current move
            if collision == True and (collisions < maxCollisions):
                collision = False
                bob.backward(1)
                maxMove = int(input("Enter maximum distance to travel between moves (20-200 pixels): "))
                direction = getNewHeading(bob)
                ...
                print(f"Check distance, distance={distance}, maxMove={maxMove}, collisions={collisions}")

    input("Press any key to exit ya doofus!")
