...
def checkRightWallCollision(turtle,wall):
    ...
    if x < wallX or y > wallY1 or y < wallY2:
        return False
    else:
...
def main():
    ...
    # set turtles' heading to the newly acquired heading and get current position
        ...
        if checkRightWallCollision(bob, leftWall):
            getNewHeading(bob)
            collisionCount += 1
        ...
        elif checkRightWallCollision(bob, rightWall):
            getNewHeading(bob)
            collisionCount += 1
    ...
    if collisionCount/guess > 1:
        print("There were less collisions than you guessed")
    elif collisionCount/guess < 1:
