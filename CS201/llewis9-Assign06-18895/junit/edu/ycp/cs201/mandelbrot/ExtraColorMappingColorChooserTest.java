...
public class ExtraColorMappingColorChooserTest {
	
	// reference for Color chooser under test
	ExtraColorMappingColorChooser chooser;
	...
	@Test
	public void testExtraColorMappingColorChooser() {
		
		System.out.println("*** Constructor Test ***");		
		
		// test constructor that creates everything - creates all 3 Maps
		chooser = new ExtraColorMappingColorChooser(iterCounts);
	...
	public void testCreateIterCountMap() {
		...
		chooser = new ExtraColorMappingColorChooser();
		
		// create iterCountMap and retrieve it (without creating other Maps)
		TreeMap<Integer, Integer> iterCountMap = chooser.createIterCountMap(iterCounts);
	...
	// test case for method that creates iterSpectrumMap
	...
	public void testCreateIterSpectrumMap() {
		...
		// create iterSpectrumMap and retrieve it (without creating other Maps)
		HashMap<Integer, Integer> iterSpectrumMap = chooser.createIterSpectrumMap();
		...
		Set<Integer> keySet     = iterSpectrumMap.keySet();
		...
		for (Integer key : testKeySet) {
			...
			assertTrue(testIterSpectrumMap.get(key) == iterSpectrumMap.get(key));
		}					
	}
	...
	public void testCreateIterColorMap() {
		...
		// create iterColorMap and retrieve it (without creating other Maps)
		HashMap<Integer, Color> iterColorMap = chooser.createIterColorMap();
		
		// # of entries in each Map must be the same
		System.out.println(testIterColorMap.size() + " == " + iterColorMap.size());
		assertTrue(testIterColorMap.size() == iterColorMap.size());
		
		
		// get key sets and place in arrays
		Set<Integer> testKeySet = testIterColorMap.keySet();
		Set<Integer> keySet     = iterColorMap.keySet();
		...
		for (Integer key : testKeySet) {
			...
			System.out.print("Expected Value: " + testIterColorMap.get(key) + ", " + "Actual Value: " + iterColorMap.get(key));
			assertTrue(testIterColorMap.containsKey(key) == iterColorMap.containsKey(key));
		}
	}
